...
__global__ void kernel(float *h, float *u, float *v, float *dh1, float *du1, float *dv1, int nx, int ny, int t, float dx, float dy, float dt, float g, float H)
{
    ...
    for (int i = threadIdx.x; i < halo_block_dims[0] * halo_block_dims[1]; i += blockDim.x)
    {
        ...
        printf("Thread %d of block (%d, %d) is loading in from grid (%d, %d) into block (%d, %d) and local idx %d. The corresponding block h value is %f.\n", threadIdx.x, blockIdx.x, blockIdx.y, grid_x, grid_y, thread_x, thread_y, local_idx, block_h(thread_x, thread_y));
    ...
    for (int i = threadIdx.x; i < block_dims[0] * block_dims[1]; i += blockDim.x)
    {
        ...
        v(grid_x, grid_y) = block_v(thread_x + BLOCK_HALO_RAD, thread_y + BLOCK_HALO_RAD);
